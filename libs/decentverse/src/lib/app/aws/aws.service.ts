...
export class AwsService {
  ...
  async getObject({ bucket, path, filename }: dto.GetObjectRequest) {
    return await this.s3.getObject({ Bucket: bucket, Key: `${path}/${filename}` }).promise();
  }
  async getJsonObject({ bucket, path, filename }: dto.GetObjectRequest) {
    const data = await this.s3.getObject({ Bucket: bucket, Key: `${path}/${filename}` }).promise();
  ...
  async uploadFile({ path, filename, rename, localPath, meta }: dto.AwsUploadRequest) {
    ...
    await this.s3
      ...
      .promise();
    return this.options.host ? this.getServiceUrl(this.options.host, key) : this.getObjectUrl(this.options.bucket, key);
  }

  async saveFile({ bucket, path, filename, localPath, rename }: dto.DownloadRequest): Promise<dto.LocalFilePath> {
    ...
    return new Promise((resolve, reject) => {
      stream.on("end", () => {
        rename && fs.renameSync(`${localPath}/${filename}`, `${localPath}/${rename}`);
        setTimeout(() => resolve({ filename: rename ?? filename, localPath }), 100);
      });
  ...
  async copyObject({ bucket, copyPath, pastePath, filename, host }: dto.CopyRequest) {
    ...
    await this.s3
      ...
      .promise();
    return host ? this.getServiceUrl(host, key) : this.getObjectUrl(bucket, key);
  }
  ...
  async makePublic({ bucket, path, filename }: { bucket: string; path: string; filename: string }) {
    await this.s3
      .putObjectAcl({
        ACL: "public-read",
        Bucket: bucket,
  ...
  async makePrivate({ bucket, path, filename }: { bucket: string; path: string; filename: string }) {
    await this.s3
      .putObjectAcl({
        ACL: "private",
        Bucket: bucket,
  ...
  getContentType(filename: string) {
    return filename.includes(".png") ? "image/jpeg" : filename.includes(".json") ? "application/json" : undefined;
  }
}
