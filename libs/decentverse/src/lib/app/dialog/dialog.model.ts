 ...
 * ? Schema Class의 이름을 변경합니다. Dialog -> ClsssName
 * ? Input은 도큐먼트를 생성할 때 입력하는 field들을 작성합니다. Prop 내에는 속성을, 필드값에는 타입을 지정합니다.
 * ? SchemaDialog은 도큐먼트를 생성 시 자동으로 생성되는 field들을 작성합니다. Prop 내 속성에는 반드시 default 필드가 있어야합니다.
...
export class Dialog extends Input {
  @Prop({
    type: String,
...
export class Raw extends Dialog {}
...
schema.pre<Doc>("save", async function (next) {
  this.flows.map((flow) => {
    if (flow.character && !this.characters.some((_id) => _id.equals(flow.character)))
      this.characters.push(flow.character);
  });
  next();
});
